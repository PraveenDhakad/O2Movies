...
class DooPlayer{
            ...
            if($typ){
                switch($typ) {
                    ...
                    case 'gdrive':
                    	...
                    	break;
	                case 'streamaly':
		                $code = "<if" . "rame class='metaframe rptss' src='//streamaly.me/{$streamalyUrl}' frameborder='0' scrolling='no' allow='autoplay; encrypted-media' allowfullscreen></ifr" . "ame>";
	...
	public function checkTv( $post ) {
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$streamaly = 'play/'.$ids . '-' . $post['temporada'] . '-' . $post['episodio'] . $animeo;
		}
