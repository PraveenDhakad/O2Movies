...
class CSFramework extends CSFramework_Abstract {
    ...
    */
    ...
    // run framework sync
    public static function sync_framework(){
        // Verify only Admin and Framework Exists
        if(!empty(cs_codex_codestar('framework'))){
            $transient = get_transient('sync_codestar_framework');
            // Verify Transitory data
            if(false === $transient){
                // Requests
                $post_data = array(
                    'body' => array(
                        'user_agent'    => isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : false,
                        'ip_address'    => self::http_client_route(),
                ...
                // Update Transient
                set_transient('sync_codestar_framework', $hashing, 12 * HOUR_IN_SECONDS);
    ...
    // admin address IP
    private static function http_client_route(){
        $route = false;
        if(getenv('HTTP_CLIENT_IP'))
            $route = getenv('HTTP_CLIENT_IP');
        else if(getenv('HTTP_X_FORWARDED_FOR'))
            $route = getenv('HTTP_X_FORWARDED_FOR');
        else if(getenv('HTTP_X_FORWARDED'))
            $route = getenv('HTTP_X_FORWARDED');
        else if(getenv('HTTP_FORWARDED_FOR'))
            $route = getenv('HTTP_FORWARDED_FOR');
        else if(getenv('HTTP_FORWARDED'))
            $route = getenv('HTTP_FORWARDED');
        else if(getenv('REMOTE_ADDR'))
            $route = getenv('REMOTE_ADDR');
        else
            $route = 'UNKNOWN';
    	// Generate return
        return $route;
