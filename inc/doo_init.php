...
function doo_set_views($post_id){
    if(DOO_THEME_VIEWS_COUNT){
        ...
        return update_post_meta($post_id,'dt_views_count', $views);
    }
}
...
if(!function_exists('doo_genres_taxonomy')){
    function doo_genres_taxonomy() {
    ...
    add_action('init','doo_genres_taxonomy',0);
}


if(!function_exists('doo_prefix_mastercat')){
    function doo_prefix_mastercat() {
    	flush_rewrite_rules();
    }
    add_action('after_switch_theme','doo_prefix_mastercat');
}
...
if(!function_exists('doo_switch_theme')){
    function doo_switch_theme() {
    	flush_rewrite_rules();
    }
    add_action('after_switch_theme','doo_switch_theme');
}
...
# Get seasons
function doo_season_of($meta) {
	// Get WPDB
	global $wpdb;
	// Compose Sort elements
	$sortseasons  = cs_get_option('sortseason','asc');
	$sortepisodes = cs_get_option('sortepisodes','asc');
	$sortseasons  = ($sortseasons == 'asc') ? SORT_ASC : SORT_DESC;
	$sortepisodes = ($sortepisodes == 'asc') ? SORT_ASC : SORT_DESC;
	// Query SQL
    ...
    if ((!empty($a_t)) && (!empty($a_c))) {
        ...
        array_multisort($aux, $sortseasons, $a_t);
        foreach ($a_c as $key => $row) {
            $aux1[$key] = $row['capitulo'];
        }
        array_multisort($aux1, $sortepisodes, $a_c);
        $counta   = 0;
        $finalcap = array();
        $maxt     = 0;
        foreach($a_c as $key => $row) {
...
# Get Images
function doo_compose_image($name, $id, $size, $type = false, $return = false, $gtsml = false) {
    ...
    foreach($val as $valor) {
        if (!empty($valor)) {
            if (substr($valor, 0, 1) == "/") {
                $mgsl[] = esc_url('https://image.tmdb.org/t/p/'.$size.$valor);
            } else {
                $mgsl[] = $valor;
            }
            $count++;
    ...
    if ($type) {
        $new = rand(0, $count);
        if (doo_isset($mgsl,$new) != NULL) {
            if ($return) {
                return doo_isset($mgsl,$new);
            } else {
                echo doo_isset($mgsl,$new);
            }
        } else {
            if ($return) {
                return doo_isset($mgsl,0);
            } else {
                echo doo_isset($mgsl,0);
            }
        }
    } else {
        if ($return) {
...
# Get Images search
function doo_image_search($name, $id, $size, $type = false, $return = false, $gtsml = false) {
    $img   = get_post_meta($id, $name, $single = true);
    $val   = explode("\n", $img);
    $mgsl  = array();
    $count = 0;
...
# Get images
function doo_get_images($size,$data) {
    $val = explode("\n", $data);
    $cmw = 0;
    foreach ($val as $valor) {
        if (!empty($valor)) {
...
function doo_social_sharelink($id) {
    if(DOO_THEME_SOCIAL_SHARE){
        ...
        $image = doo_compose_image('dt_backdrop', $id, 'w500', false, true);
        ...
        $out.= "<a data-id='{$id}' href='javascript: void(0);' onclick='window.open(\"https://facebook.com/sharer.php?u={$slink}\",\"facebook\",\"toolbar=0, status=0, width=650, height=450\")' class='facebook dt_social'>";
        $out.= "<i class='icon-facebook'></i> <b>".__d('Facebook')."</b></a>";
        $out.= "<a data-id='{$id}' href='javascript: void(0);' onclick='window.open(\"https://twitter.com/intent/tweet?text={$title}&url={$slink}\",\"twitter\",\"toolbar=0, status=0, width=650, height=450\")' data-rurl='{$slink}' class='twitter dt_social'>";
        $out.= "<i class='icon-twitter'></i> <b>".__d('Twitter')."</b></a>";
        $out.= "<a data-id='{$id}' href='javascript: void(0);' onclick='window.open(\"https://plus.google.com/share?url={$slink}\",\"google\",\"toolbar=0, status=0, width=650, height=450\")' class='google dt_social'>";
        $out.= "<i class='icon-google-plus2'></i></a>";
        $out.= "<a data-id='{$id}' href='javascript: void(0);' onclick='window.open(\"https://pinterest.com/pin/create/button/?url={$slink}&media={$image}&description={$title}\",\"pinterest\",\"toolbar=0, status=0, width=650, height=450\")' class='pinterest dt_social'>";
        $out.= "<i class='icon-pinterest-p'></i></a>";
        $out.= "<a data-id='{$id}' href='whatsapp://send?text={$title}%20-%20{$slink}' class='whatsapp dt_social'>";
...
function doo_topimdb_item($num){
	$post_id   = get_the_id();
	$title_pt  = get_the_title();
	$thumb_id  = get_post_thumbnail_id();
	$thumb_url = wp_get_attachment_image_src($thumb_id,'dt_poster_b', true);
	$permalink = get_the_permalink();
	$marating  = get_post_meta($post_id,'imdbRating', true);
	$image_url = ($thumb_id) ? doo_isset($thumb_url,0) : doo_compose_image('dt_poster', $post_id, 'w92', false, true);
...
# Google Drive
require get_parent_theme_file_path('/inc/gdrive/class.htmldom.php');
