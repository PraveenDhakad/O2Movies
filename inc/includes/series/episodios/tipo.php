...
function episodio_table_content( $column_name, $post_id ) {
	...
	if ($column_name == 'report') {
		if($minutes = get_post_meta( $post_id, 'numreport', true )) {
			echo '<span class="dt_report_video">'. $minutes .'</spam> <a href="'.get_admin_url(get_current_blog_id(),'admin-ajax.php?action=delete_notice_report&id='.$post_id).'">'.__d('Solved').'</a>';
		} else {
			echo "0";
		}
    }
