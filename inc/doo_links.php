...
class DooLinks{
     ...
     * @version 1.1
     */
    public static function shorteners($url){
        $option = cs_get_option('shorteners');
        if(!empty($option) && is_array($option) && filter_var($url, FILTER_VALIDATE_URL)){
            $numb = array_rand($option);
            $link = $option[$numb]['short'];
            $link = str_replace('{{url}}', $url, $link);
            $link = apply_filters('doo_links_shorteners',$link);
            $murl = esc_url($link);
        }else{
            $murl = filter_var($url, FILTER_VALIDATE_URL) ? esc_url($url) : $url;
        }
        return $murl;
    }
