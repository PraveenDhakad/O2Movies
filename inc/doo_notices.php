...
class DooNotices{
    ...
    public function __construct(){
        if(is_admin()){
            $dbmovies = get_option('dbmovies_options');
            ...
            }elseif(empty(doo_isset($dbmovies,'dbmv'))){
    ...
    public function activate_dbmovies(){
    	...
    	$out .= '<span class="dashicons dashicons-warning" style="color: #00a0d2"></span> ';
    	$out .= __d('dbmovies API has not been activated, active to be able to generate content'). ' <a href="' .admin_url('admin.php?page=dbmovies').'"><strong>'. __d('Active Dbmovies now') .'</strong></a>';
        $out .= '</p></div>';
	    $out = '';
        echo $out;
    }
